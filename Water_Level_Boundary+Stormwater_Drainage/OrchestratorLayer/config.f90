...
! #endif
    ...
    !if ( io_config_outputs .eq. 4 ) RTOUT_DOMAIN = 0 ! CORRECTION BY YSON FOR OUTPUT (.RTOUT) OPTIMIZATION OF IO_CONFIG_OUTPUTS = 4 (HYDRO.NAMELIST)
    ...
    IF(MOD(NINT(NLST(DID)%DT * 1000), NINT(DTRT_TER * 1000)) .NE. 0.0) THEN ! CORRECTION BY YSON: DTRT_TER can be smaller than 1
    !if(nlst(did)%DT/DTRT_TER .ne. real(int(nlst(did)%DT) / int(DTRT_TER)) ) then ! CORRECTION BY YSON: DTRT_TER can be smaller than 1
         print*, "nlst(did)%DT,  DTRT_TER = ",nlst(did)%DT,  DTRT_TER
         call hydro_stop("module_namelist: DT not a multiple of DTRT_TER")
    endif
    IF(MOD(NINT(NLST(DID)%DT * 1000), NINT(DTRT_CH * 1000)) .NE. 0.0) THEN ! CORRECTION BY YSON: DTRT_CH can be smaller than 1
    !if(nlst(did)%DT/DTRT_CH .ne. real(int(nlst(did)%DT) / int(DTRT_CH)) ) then ! CORRECTION BY YSON: DTRT_CH can be smaller than 1
         print*, "nlst(did)%DT,  DTRT_CH = ",nlst(did)%DT,  DTRT_CH
